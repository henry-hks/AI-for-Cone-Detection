...
def depth_cone_detect(depthimg, net, name):
  image_board = depthimg
  name = [name]
  image_board, detections = dni.image_detection_(image_board, net, name, 0.5)

  return detections


def cone_detect(image, mask, net, class_colors, name):
  name = [name]
  image_for = image
